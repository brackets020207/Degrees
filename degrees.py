...
def shortest_path(source, target):
    ...
    qfrontier.add(current)
    explored.add(current)
    while len(qfrontier.frontier) > 0:
        #print([node.state for node in qfrontier.frontier])
        current = qfrontier.remove()
        #print(current.state)
        if current.state == target:
            ...
            for movie_id, person_id in neighbors_for_person(current.state):
                if person_id in explored:
                    ...
                    n_node = Node(person_id, current, movie_id)
                    explored.add(person_id)
                    qfrontier.add(n_node)
